...
因此，结区节点掺杂及三角形内线性插值会通过平衡载流子、SG population
derivative 和耗尽区宽度影响软模态，是合理的放大因素；但当前数据不支持把
“补偿结区三角形的掺杂平滑公式”认定为唯一或直接错误。更准确的空间定位是
结区肩部/耗尽区全局模态，而非单个三角形的局部平均。

...
AUDIT = ROOT / "build-release" / "pn2d-forward-field-audit-charge-area-fix-20260727"
SENTAURUS_AUDIT = ROOT / "build-release" / "pn2d-forward-field-audit-20260727"
MESH_PATH = (
    ROOT
    / "build-release"
...
def vela_path(bias: float) -> Path:
    ...
    token = f"{bias:g}V"
    return AUDIT / f"vela_forward_fields_fixed_{index:04d}_{token}.vtk"


def sent_field(bias: float, name: str) -> dict[int, float]:
    token = f"{bias:g}v"
    return read_scalar(SENTAURUS_AUDIT / "sentaurus_fields" / token / "fields" / f"{name}_region0.csv")
